...
void VariableManager::addPointer(void *ptr, std::string dataType, std::string variableName, std::string pointerType) {
    Scope* list = scopes.front();
    list->addPointer(ptr, dataType, variableName, pointerType);
}
...
std::string VariableManager::endScope(){
    ...
    if (lastScope != mainScope){
        ...
        return "MemoryManagement Error";
    }
}

/*
 * Crea un MemoryManagement
 */
...
std::string VariableManager::returnVariableValue(std::string jsonString) {
    ...
    jsonVariable["tipoDeDato"] = variableNode->getDataType();
...
std::string VariableManager::returnStructAttribute(std::string jsonString) {
    ...
    std::string name = jsonObject.get("nombre", "NameError").asString();
    Node* node = searchNodeOfStruct(structName, name);
...
Node *VariableManager::searchNodeOfStruct(std::string structName, std::string name) {
    Node* nodo = nullptr;
    for (auto list : scopes){
        nodo = list->searchNodeOfStruct(structName, name);
        if (nodo)
            break;
    }
    return nodo;
